...
for ax, year in zip(axes, years):
    ...
    for cluster, group in year_selections.groupby('cluster'):
        ...
        # log_black_counts = np.log1p(group["black_population"].astype(float).to_numpy())
        # max_log_black = np.log1p(CBSA_selections["black_population"].max())
        black_count = group["black_population"].astype(float).to_numpy()
        max_black_count = CBSA_selections["black_population"].max()
        
        nx.draw_networkx_edges(selected_graph, positions, ax=ax, edge_color="black",
            width=0.5, alpha=0.45)
        nx.draw_networkx_nodes(selected_graph, positions, ax=ax, nodelist=selected_nodes,
                        node_color=black_count, cmap="Blues", vmin=0, 
                        vmax=max_black_count, node_size=3)

        centroid_gisjoin = CBSA_metrics.loc[
            (CBSA_metrics['year'] == year) & (CBSA_metrics['cluster'] == cluster),
            'center_gisjoin'].iloc[0]
        centroid_black_population = CBSA_metrics.loc[
            (CBSA_metrics['year'] == year) & (CBSA_metrics['cluster'] == cluster),
            'black_population'].iloc[0]
        centroid_black_share = CBSA_metrics.loc[
            (CBSA_metrics['year'] == year) & (CBSA_metrics['cluster'] == cluster),
            'black_share'].iloc[0]
